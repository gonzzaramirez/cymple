...
import {
  ...
  markOrgDepositPaid,
  cancelOrgBooking,
  manualConfirmOrgBooking,
  updateOrgBookingNotes,
} from "@/lib/api/organization-bookings";
import {
  BookingsFilters,
  BookingsList,
  BookingDetailDialog,
...
export default function CenterBookingsPage() {
  ...
  async function loadBookings() {
    ...
    setError(null);
    try {
      const data = await listOrgBookings({ status, month, year, page, limit: 10 });
  ...
  const handleSelect = useCallback(async (booking: BookingSummary) => {
    ...
    setDetailOpen(true);
    try {
      const detail = await getOrgBookingDetail(booking.id);
  ...
  const refreshWithPage = useCallback(
    (currentPage: number) => {
      void listOrgBookings({ status, month, year, page: currentPage, limit: 10 }).then(
  ...
  const handleDepositPaid = useCallback(() => {
    if (selectedBooking) {
      getOrgBookingDetail(selectedBooking.id).then(setDetailData).catch(() => {});
    }
    ...
    <section className="space-y-6">
      <div>
        ...
        <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
          Gestioná todas las reservas online de tus profesionales desde un solo lugar.
        </p>
      </div>
      ...
      {detailData && (
        <BookingDetailDialog
          ...
          apis={{
            markDepositPaid: markOrgDepositPaid,
            cancelBooking: cancelOrgBooking,
            manualConfirm: manualConfirmOrgBooking,
            updateNotes: updateOrgBookingNotes,
          }}
        />
      )}
