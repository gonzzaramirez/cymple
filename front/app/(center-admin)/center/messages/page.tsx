...
export default async function CenterMessagesPage({
  searchParams,
}: {
    ...
    <section className="space-y-6">
      ...
      <Card className="shadow-card">
        <CardContent className="pt-6">
          <form className="flex flex-wrap items-end gap-3">
            ...
            {sp.search || sp.dateFrom || sp.dateTo ? (
              <Link
                href="/center/messages"
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {data.items.length === 0 ? (
          ...
          data.items.map((item) => (
            <Link key={item.patientId} href={`/center/messages/${item.patientId}`}>
      ...
      {data.totalPages > 1 ? (
        <div className="flex items-center justify-center gap-4 pt-2">
          {page > 1 ? (
            <Link
              href={`/center/messages?${buildQuery({ page: String(page - 1), search: sp.search, dateFrom: sp.dateFrom, dateTo: sp.dateTo })}`}
              className={cn(
